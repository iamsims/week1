...
template sudoku() {
    ...
    //[assignment] hint: you will need to initialize your RangeProof components here
     ...
     component rangeproof_puzzle[9][9];
      component rangeproof_solution[9][9];
      
      for (var i=0; i<9; i++) {
          for (var j=0; j<9; j++) {
	      rangeproof_puzzle[i][j] = RangeProof(32);              
              rangeproof_puzzle[i][j].in <== puzzle[i][j];
              rangeproof_puzzle[i][j].range[0] <== 0;
              rangeproof_puzzle[i][j].range[1] <== 9;
              
              assert(rangeproof_puzzle[i][j].out == 1 );
              
	      rangeproof_solution[i][j] = RangeProof(32);              
              rangeproof_solution[i][j].in <== solution[i][j];
              rangeproof_solution[i][j].range[0] <== 0;
              rangeproof_solution[i][j].range[1] <== 9;
              
              assert(rangeproof_solutione[i][j].out == 1);
